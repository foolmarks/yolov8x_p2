...
class MyPlugin(AggregatorTemplate):
    ...
    def run(self, input_buffers: List[SimaaiPythonBuffer], output_buffer: bytes) -> None:
        ...
        orig_image = np.frombuffer(input_buffers[1].data, dtype=np.uint8, count=H*W*C).reshape(H,W,C)
        ...
        annotated = utils.draw_detections(img_bgr, boxes_640, scores, class_ids, utils.COCO_CLASSES)
