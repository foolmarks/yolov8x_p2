...
def _data_prep(
    folder_path: str, num_images: int, input_shapes_dict: Dict
) -> List[Dict]:
    """
    Prepare data
    """
    ...
    for input_name in input_shapes_dict.keys():
        for img_path in image_paths:
            ...
            if img_bgr is None:
                ...
                continue
            data, _ = utils.preprocess_image(img_bgr, do_transpose=False)
            processed_data.append({input_name: data})
...
def implement(args):
    ...
    if args.bf16:
        weights_quant_scheme = QuantizationScheme(
            asymmetric=False, per_channel=False, bf16=True
        )
        activ_quant_scheme = QuantizationScheme(
            asymmetric=False, per_channel=False, bf16=True
        )
    else:
        weights_quant_scheme = QuantizationScheme(
        ...
        activ_quant_scheme = QuantizationScheme(
            asymmetric=True, per_channel=False, bits=args.quant_bits
        )

    # set other quantization parameters
    quant_config = (
        default_quantization.with_activation_quantization(activ_quant_scheme)
    ...
    print(
        f"Wrote compiled model to {results_dir}/{output_model_name}_mpk.tar.gz",
        flush=True,
    )
    ...
    with tarfile.open(f"{results_dir}/{output_model_name}_mpk.tar.gz") as tar:
        ...
        tar.extract("0_boxdecoder.json", f"{results_dir}")
...
def run_main():
    ...
    ap.add_argument(
        ...
        type=int,
        default=1,
        help="requested batch size. Default is 1",
    )
    ap.add_argument(
        "-om",
        "--output_model_name",
        ...
        help="Output model name. Default is yolov8x-p2_opt_4o",
    )
    ap.add_argument(
        "-cd",
        "--calib_dir",
        ...
        help="Path to calib images folder. Default is ./calib_images",
    )
    ap.add_argument(
        "-td",
        "--test_dir",
    ...
    ap.add_argument(
        ...
        default=2,
        choices=[1, 2],
        help="Target device: 1 = DaVinci, 2 = Modalix. Default is 2",
    )
    ap.add_argument(
        "-e",
        "--evaluate",
        action="store_true",
        default=False,
        help="If set, evaluate the quantized model",
    )
    ap.add_argument(
        "-cm",
        "--calib_method",
        type=str,
        default="min_max",
        choices=["mse", "min_max", "moving_average", "entropy", "percentile"],
        help="Calibration method. Default is min_max",
    )
    ap.add_argument(
        "-qb",
        "--quant_bits",
        ...
        default=8,
        choices=[8, 16],
        help="Bits for quantization. Default is 8bits",
    )
    ap.add_argument(
        "-bc",
        "--bias_corr",
        action="store_true",
        help="Use bias correction. Default is no bias correction",
    )
    ap.add_argument(
        "-ce",
        "--chan_equal",
        action="store_true",
        help="Use channel equalization. Default is no channel equalization",
    )
    ap.add_argument(
        "-bf16",
        "--bf16",
