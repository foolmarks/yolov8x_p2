...
def preprocess_image(img_bgr: np.ndarray, do_transpose: bool = True) -> np.ndarray:
    ...
    # BGR -> RGB
    img_rgb = cv2.cvtColor(bgr_padded, cv2.COLOR_BGR2RGB)
    ...
    img_rgb = np.expand_dims(img_rgb, axis=0)  # (1,3,H,W) or (1,H,W,3)

    return img_rgb, bgr_padded
