...
class UserContext {
  ...
  static std::atomic<int> global_instance_counter;
public:
  UserContext(const char *json_file_name);
  int instance_id;
  ~UserContext();
...
static GstCaps *
GS_FUNC(_fixate_src_caps)(GstAggregator *aggregator, GstCaps *caps)
{
  GSP *self = GST_SIMAAI_AGG_TEMPLATE(aggregator);

  return gst_simaai_caps_fixate_src_caps(GST_ELEMENT(self),
    self->priv->simaai_caps, caps);
}

static gboolean
GS_FUNC(_negotiate)(GstAggregator *aggregator)
{
  GSP *self = GST_SIMAAI_AGG_TEMPLATE(aggregator);

  return gst_simaai_caps_negotiate(GST_ELEMENT(self), self->priv->simaai_caps);
}

static gboolean
GS_FUNC(_sink_query)(GstAggregator *aggregator,
  GstAggregatorPad *aggregator_pad, GstQuery *query)
{
  ...
  switch (GST_QUERY_TYPE(query)) {
    case GST_QUERY_CAPS:
      return gst_simaai_caps_query(GST_ELEMENT(self),
        self->priv->simaai_caps, query, GST_PAD_SINK);
  }

  return GST_AGGREGATOR_CLASS(GS_FUNC(_parent_class))->sink_query(aggregator,
    aggregator_pad, query);
}

static gboolean
GS_FUNC(_src_query)(GstAggregator *aggregator, GstQuery *query)
{
  GSP *self = GST_SIMAAI_AGG_TEMPLATE(aggregator);
  ...
  switch (GST_QUERY_TYPE(query)) {
    case GST_QUERY_CAPS:
      return gst_simaai_caps_query(GST_ELEMENT(self),
        self->priv->simaai_caps, query, GST_PAD_SRC);
  }

  return GST_AGGREGATOR_CLASS(GS_FUNC(_parent_class))->src_query(aggregator,
    query);
}
  ...
  switch (transition) {
    ...
    case GST_STATE_CHANGE_NULL_TO_READY: {
      if (!gst_simaai_caps_parse_config(GST_ELEMENT(self),
        self->priv->simaai_caps, self->priv->config_file->str)) {
      ...
      self->priv->user_context = new UserContext(self->priv->config_file->str);
    } break;
...
static void GS_FUNC(_class_init)(GSClass *klass) {
  ...
  sink_pad_template = gst_pad_template_new_with_gtype(PAD_TEMPLATE_NAME_SINK,
    GST_PAD_SINK, GST_PAD_REQUEST, gst_caps_new_any(), GST_TYPE_AGGREGATOR_PAD);
  ...
  src_pad_template = gst_pad_template_new_with_gtype(PAD_TEMPLATE_NAME_SRC,
    GST_PAD_SRC, GST_PAD_ALWAYS, gst_caps_new_any(), GST_TYPE_AGGREGATOR_PAD);
  if (!src_pad_template) {
  ...
  base_aggregator_class->src_query = GST_DEBUG_FUNCPTR(GS_FUNC(_src_query));
  base_aggregator_class->negotiate = GST_DEBUG_FUNCPTR(GS_FUNC(_negotiate));
