...
class MyPlugin(AggregatorTemplate):
    ...
    def run(
        ...
        if ok:
            print("Wrote PNG file to /tmp/output.png")

        # convert to I420
        img_yuv = cv2.cvtColor(annotated, cv2.COLOR_BGR2YUV_I420)
        print("I420 image shape:", img_yuv.shape)

        # output
        view = memoryview(img_yuv)
        nbytes = img_yuv.nbytes
        output_buffer[:nbytes] = view.cast("B")
